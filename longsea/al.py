...
def df_comb_cols(df, cols, name):
    '''
    将指定列的数据合并为新列，并返回新DF

    :param df: 输入的数据框，包含需要处理的数据
    :param cols: 需要处理的列名列表
    :param name: 处理后生成的新列的名称
    :return: 处理后的数据框
    '''
    ...
    return df
def df_marry_cols(df, cols, marry):
    '''
    将指定cols中的每一列与marry合并，并返回。

    :param df: 输入的数据框，包含待处理的数据
    :param cols: 需要处理的列名列表，
    :param marry: 需要合与之合并的列，
    :return: 处理后的数据框
    '''
    ...
    if not isinstance(cols, list):
        raise TypeError("cols必须是列表类型")
    if not isinstance(marry, str) or not marry:
        raise ValueError("marry必须是非空字符串")
    ...
    # 使用更高效的方式组合列
    for col in cols:
        df[col] = list(zip(df[col], df[marry]))
    return df
def merge_multiple_dfs(dfs, on, how='outer', keep_last=True):
    """
    合并多个DataFrame，处理重复列名
    ...
    注意事项:
    - 所有DataFrame必须包含on参数指定的列
    - 合并键的数据类型在所有DataFrame中应该一致
    - 使用how='left'或how='right'时，结果可能受合并顺序影响
    """
    # 验证输入
    if not dfs:
        return pd.DataFrame()
    ...
    return reduce(merge_two_dfs, dfs)
def df_sort(df, cols, idx='A'):
    """
    将DataFrame的列按指定顺序排序，然后按指定列进行行排序

    :param df: 要排序的DataFrame
    :param cols: 列顺序列表
    :param idx: 用于行排序的列名，默认为'A'
    :return: 排序后的DataFrame
    """
    # 确保指定的排序列存在于DataFrame中
    if idx not in df.columns:
        raise ValueError(f"DataFrame必须包含'{idx}'列")

    # 检查cols是否包含所有列
    missing_cols = set(df.columns) - set(cols)
    extra_cols = set(cols) - set(df.columns)

    if missing_cols:
        print(f"警告: cols中缺少以下列: {missing_cols}")

    if extra_cols:
        print(f"警告: cols中包含DataFrame中不存在的列: {extra_cols}")

    # 按指定顺序排列列，同时保留不在cols中的列
    ordered_cols = [col for col in cols if col in df.columns]
    ...
    # 按指定列排序
    df_sorted = df_sorted.sort_values(by=idx)
